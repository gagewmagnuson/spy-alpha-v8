...
Architecture (from build spec Section 6):
    Stage 1: Strategy Activation
        - For each strategy, compute activation score in [0, 1]
        - Answers: "Should this strategy matter right now?"
        - Smooth activation with exponential decay (alpha = 0.3)
...
TRAIN_WINDOW: int = 2520          # ~10 years
RETRAIN_EVERY: int = 126          # ~semi-annually
...
def merge_extended_features(
    ...
    etf_era_start: str = "2005-01-01",
) -> pd.DataFrame:
    """
    Merge extended dataset features (1970+) with ETF-era features.

    Pre-2005: uses extended features only (state geometry learning)
    Post-2005: uses ETF-era features (full resolution with strategy data)

    Adds era encoding and sample age features so the model can
    distinguish pre-ETF from post-ETF market structure.
    """
    # Use extended features for pre-ETF era
    etf_start = pd.Timestamp(etf_era_start)
    pre_etf = extended_features.loc[extended_features.index < etf_start].copy()

    # For post-ETF, use the ETF-era features (higher quality)
    # But also include any extended feature columns not in ETF features
    post_etf = etf_features.copy()

    # Find columns that exist in extended but not in ETF features
    extended_only_cols = [c for c in extended_features.columns if c not in etf_features.columns]

    if extended_only_cols:
        # Add extended-only columns to post-ETF period
        ext_post = extended_features.loc[extended_features.index >= etf_start, extended_only_cols]
        post_etf = pd.concat([post_etf, ext_post.reindex(post_etf.index)], axis=1)

    # Combine pre-ETF and post-ETF
    # For pre-ETF rows, columns that only exist in ETF features will be NaN
    combined = pd.concat([pre_etf, post_etf], axis=0)
    combined = combined.sort_index()
    combined = combined.loc[~combined.index.duplicated(keep='last')]

    # ---- Era encoding ----
    combined["era_post_etf"] = (combined.index >= etf_start).astype(float)

    # ---- Sample age feature (years since observation) ----
    latest = combined.index[-1]
    combined["sample_age_years"] = (latest - combined.index).days / 365.25

    logger.info(
        f"Merged features: {combined.shape[0]} days x {combined.shape[1]} columns, "
        f"pre-ETF: {(combined.index < etf_start).sum()} days, "
        f"post-ETF: {(combined.index >= etf_start).sum()} days"
    )
...
def compute_sample_weights(
    ...
    etf_era_start: str = "2005-01-01",
    decay_half_life_years: float = 15.0,
) -> pd.Series:
    """
    Compute sample weights that decay with age.

    Post-ETF samples get full weight. Pre-ETF samples get exponentially
    decayed weight based on their age. This ensures modern data dominates
    while pre-ETF data still contributes macro pattern learning.

    half_life_years=15 means a sample from 15 years before the ETF era
    gets 50% weight, 30 years before gets 25%, etc.
    """
    etf_start = pd.Timestamp(etf_era_start)
    latest = index[-1]
    ...
    pre_etf_mask = index < etf_start
    if pre_etf_mask.any():
        # Years before ETF era start
        years_before = (etf_start - index[pre_etf_mask]).days / 365.25

        # Exponential decay
        decay_rate = np.log(2) / decay_half_life_years
        weights.loc[pre_etf_mask] = np.exp(-decay_rate * years_before)

    logger.info(
        f"Sample weights: pre-ETF mean={weights.loc[pre_etf_mask].mean():.3f}, "
        f"post-ETF mean={weights.loc[~pre_etf_mask].mean():.3f}"
    )
...
class MetaAllocator:
    ...
    def train_walk_forward(
        ...
        forward_window = 63  # evaluate strategy performance over ~3 months
        strategy_rewards = {}
        ...
        for window_start in range(start_idx, end_idx, self.retrain_every):
            ...
            for name in self.strategy_names:
                ...
                for seed in range(self.n_ensemble):
                    ...
                    if sample_weights is not None:
                        sw = sample_weights.reindex(X_tr.index)
                        if sw.notna().sum() > 0:
                            fit_kwargs["sample_weight"] = sw.fillna(0.5).values
            ...
            # ---- Stage 2: Capital Sizing ----
            # Normalize activations to sum to 1 (capital budget)
            act_sum = activations.sum(axis=1)
            capital_weights = activations.div(act_sum.replace(0, 1), axis=0)
    ...
    def _reward_to_activation(
        ...
        Uses percentile-based mapping against the training distribution:
            - Predicted reward at training median -> activation 0.5
            - Top 10% of training -> activation ~0.9
            - Bottom 10% of training -> activation ~0.1
        """
        ...
        # Compute percentile of each prediction relative to training
        activations = np.array([
            np.searchsorted(np.sort(train_vals), pred) / len(train_vals)
            for pred in predicted_rewards
        ])
...
def backtest_meta_allocator(
    ...
    extended_snapshot: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """
    ...
    # ---- Step 3b: Merge extended features if available ----
    sample_weights = None
    if extended_snapshot is not None:
        ext_features = extended_snapshot.get("extended_features", pd.DataFrame())
        if not ext_features.empty:
            logger.info("Merging extended dataset features...")
            features = merge_extended_features(features, ext_features)
            sample_weights = compute_sample_weights(features.index)
...
def print_allocator_report(results: Dict[str, Any]) -> None:
    ...
    if "error" not in meta and "error" not in equal:
        ...
        print(f"  Overall Step 6 gate:        {'PASS' if (sharpe_pass and dd_pass) else 'NEEDS REVIEW'}")